...
public class specialOrders implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
            try {
                ...
                rs=connection.createStatement().executeQuery("SELECT * FROM productscommand where idOfTheProvider="+id);
                while(rs.next()) observableList.add(new tableItem(rs.getString(2),rs.getInt(5)));
    ...
    public void confirm(ActionEvent event) throws SQLException {
        for (tableItem item: removeList){
            PreparedStatement pr=connection.prepareStatement("Insert into ProductsCommand SET existedProduct=1 WHERE idOfTheProvider=? AND ProductName=?");
            pr.setInt(1,id);
            pr.setString(2,item.getProduct());
            pr.executeUpdate();
