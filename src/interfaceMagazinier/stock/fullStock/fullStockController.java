...
public class fullStockController implements Initializable {
    ...
    ObservableList<product> searchInFullStockTable(String barcode) throws SQLException {
        ...
        try {
            ...
            while (rs.next()) {
                product newProduct;
                try {
                    newProduct = new product(rs.getString("name"), rs.getString("barcode"), rs.getFloat("buyprice"), rs.getFloat("sellprice"), rs.getInt("quantity"), rs.getDate("expirationdate").toString());
                } catch (Exception e) {
                    newProduct = new product(rs.getString("name"), rs.getString("barcode"), rs.getFloat("buyprice"), rs.getFloat("sellprice"), rs.getInt("quantity"), "");
                }
    ...
    void overriding(product theFullStockProduct, product theFullStockApplyProduct) throws SQLException {
        ...
        preparedStatement.setString(8, theFullStockProduct.getBarcode());
        ...
        preparedStatement.setInt(7, user.getUserID());
        preparedStatement.setString(8, theFullStockApplyProduct.getBarcode());
        preparedStatement.executeUpdate();
