...
public class imDashboardController implements Initializable {
    ...
    private void loadHistory() {
        ...
        try {
            ...
            while (rs.next()){
                int id =  rs.getInt("id");
                product soldProduct = new product(String.valueOf(rs.getInt("productCode")), rs.getFloat("sellPrice"), rs.getFloat("profit"), rs.getInt("quantity"));
                if (sells.size() != 0 && sells.get(sells.size() - 1).getId() == id) {
                    ...
                    if (Integer.valueOf(soldProduct.getBarcode()) != 0) lastSellAdded.getSoldProducts().add(soldProduct);
    ...
    public void loadStatistiqueFrames(){
        ...
        try {
            String query = "Select sum(profit) from sells WHERE sellTime > date('now') and userID=?" ;
            PreparedStatement preparedStatement = connection.prepareStatement(query);
            preparedStatement.setInt(1,user.getUserID());
            ResultSet rs = preparedStatement.executeQuery();
            rs.next();
