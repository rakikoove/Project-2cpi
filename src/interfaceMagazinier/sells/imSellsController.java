    ...
    public void okClick() throws SQLException {
        try {
            if (barcodeLabel.getText().equals("")) {
                ...
                preparedStatement.setString(1, barcodeLabel.getText());
                ...
                if (rs.next()) {
                    ...
                    if (rs.getDate("expirationdate") == null)
                        newProduct = new product(rs.getString("name"), rs.getString("barcode"), rs.getFloat("buyprice"), rs.getFloat("sellprice"), rs.getInt("quantity"), "", rs.getInt("numberOfSells"));
                    else {
                        newProduct = new product(rs.getString("name"), rs.getString("barcode"), rs.getFloat("buyprice"), rs.getFloat("sellprice"), rs.getInt("quantity"), rs.getDate("expirationdate").toString(), rs.getInt("numberOfSells"));
                    }
